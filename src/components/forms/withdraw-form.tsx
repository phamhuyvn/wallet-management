...
export function WithdrawForm({ accounts, onSuccess }: { accounts: AccountOption[]; onSuccess?: () => void }) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        setError(body.error?.message ?? 'Withdraw failed');
        return;
      }
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <div className="space-y-2">
        <Label htmlFor="withdraw-amount">Amount (VND)</Label>
        <Input
          id="withdraw-amount"
          inputMode="numeric"
          pattern="^[0-9]+$"
        ...
        />
        <p className="text-xs text-slate-500">Enter the amount to withdraw (positive number).</p>
      </div>
      <div className="space-y-2">
